...
global tMagnetDetection dMahaAll ;
...
figure; 
plot(t,nbMagnetsDetected,'.') ;
xlabel('t (s)');
title('Number of magnets detected at each time step.');
zoom on; grid on;

% Calculate odometry error:

travDistance
finalOdometryErrorInPercent = (norm(X(1:2)-Xodom(1:2)) / travDistance )*100 
...
for n = 1 : nbReedSensors ,
    for k = 1 : numel(sensorReadings) ,
        if rawMeas(n,k) == 0 ;
            hold on ;
            line([k k],[n-0.5 n+0.5],'Color','b','LineStyle','-');
        end
    end
end
grid on ; zoom on;